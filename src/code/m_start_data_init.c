...
void famicom_emu_initial_common_data(void) {
    FamicomEmuCommonData* famicom = &common_data.save.flash.famicomEmuCommonData;
...
void title_game_haniwa_data_init(void) {
    ...
    for (i = 0; i < mHS_HOUSE_NUM; i++) {
        haniwa = &common_data.save.flash.homes[i].haniwa;

        mem_copy(common_data.save.flash.homes[i].haniwa.message, (u8*)haniwaBuf, HANIWA_MESSAGE_LEN);
...
void mSDI_ClearMoneyPlayerHomeStationBlock(void) {
    ...
    for (i = 0; i < 2; i++) {
        ...
        items = common_data.save.flash.foreground[blockZ][blockX].items[0];
            ...
            for (utZ = 0; utZ < UT_Z_NUM; utZ++) {
                for (utX = 0; utX < UT_X_NUM; utX++) {
                    ...
                    if (item >= ITM_MONEY_START && item <= ITM_MONEY_END) {
                        u16* deposit = common_data.save.flash.deposit[depositOffset];
...
void mSDI_PullTree(void) {
    ...
    for (blockZ = 0; blockZ < FG_BLOCK_Z_NUM; blockZ++) {
        ...
        fgBlock = &common_data.save.flash.foreground[blockZ][0];
...
void mSDI_PullTreeUnderPlayerBlock(void) {
    ...
    u16* items = &common_data.save.flash.foreground[2][2].items[0][0];
...
s32 mSDI_StartInitNew(Game* game2, s32 playerNumber, s32 mallocFlag) {
    ...
    privateInfo = &common_data.save.flash.saveFilePrivateInfo[playerNumber];
    common_data.privateInfo = privateInfo;
    common_data.playerNumber = playerNumber;

    common_data.privateInfo->gender = mPr_SEX_MALE;

    decide_fruit(&common_data.save.flash.fruit);
    if (mallocFlag == 0) {
        game = game2;
    }

    bzero(&common_data.save.flash.deposit, sizeof(common_data.save.flash.deposit));
    ...
    privateInfoPointer = &common_data.save.flash.saveFilePrivateInfo[0];

    mMld_SetDefaultMelody();
    mLd_LandDataInit();
    mEv_ClearEventSaveInfo(&common_data.save.flash.eventSaveInfo);
    ...
    mNpc_SetRemoveAnimalNo(common_data.save.flash.animals);
    title_game_haniwa_data_init();
    mPB_police_box_init(game);
    mSN_snowman_init();
    mHS_house_init();

    lbRTC_TimeCopy(&common_data.save.flash.lastGrowTime, &mTM_rtcTime_clear_code);
    lbRTC_TimeCopy(&common_data.save.flash.unk_0F89C, &mTM_rtcTime_clear_code);
    lbRTC_TimeCopy(&common_data.save.flash.unk_0F8A4, &mTM_rtcTime_clear_code);

    for (i = 0; i < PLAYER_NUM; i++) {
        common_data.save.flash.homes[i].unk_022.unk_22_0 = 0;
        common_data.save.flash.homes[i].unk_022.unk_22_2 = 0;
        common_data.save.flash.homes[i].unk_022.unk_22_4 = 0;
        common_data.save.flash.homes[i].unk_024 = i;
    }

    common_data.save.flash.stationType = RANDOM_F(15);

    for (i = 0; i < PLAYER_NUM; i++) {
        mPr_ClearMotherMailInfo(&common_data.save.flash.motherMailInfo[i]);
    }
...
void mSDI_StartInitAfter(Game* game, s32 renewalReserveFlag, s32 mallocFlag) {
    ...
    mNpc_InitNpcData();
    mNpc_InitNpcList();
    mNpc_SetNpcList(common_data.npclist, common_data.save.flash.animals, ANIMAL_NUM_MAX, mallocFlag);
    mNpc_ClearTalkInfo();
    ...
    func_800A4D10_jp();
    mSN_decide_msg();
    mPr_RenewalMapInfo(common_data.privateInfo->maps, mPr_FOREIGN_MAP_COUNT, &common_data.save.flash.landInfo);
}
