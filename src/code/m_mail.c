...
s32 mMl_hunt_for_send_address(Mail_c* mail) {
    ...
    for (i = 0; i < ARRAY_COUNT(common_data.save.flash.homes); i++) {
        if (mPr_CheckCmpPersonalID(&common_data.save.flash.homes[i].ownerID, &mail->header.recipient.personalID) == 1) {
            return i;
        }
    }
