...
s32 mNpcW_ChangeNpcWalk(NpcWalking* walk, NpcWalkInfo* info) {
    ...
    if (free_idx != -1) {
        ...
        idx = mNpcW_DecideNpc(common_data.save.flash.animals, walk->idxUse);
        ...
        mNpcW_SetNpcWalkInfo(free_info, &common_data.save.flash.animals[idx], idx);
        walk->idxUse |= (1 << idx);
...
s32 mNpcW_CheckDiffBlockWalkNpcHome(s32 blockX, s32 blockZ, NpcWalkInfo* info) {
    ...
    for (i = 0; i < NPCW_MAX; i++) {
        idx = info->idx;
        if (idx != -1) {
            home = &common_data.save.flash.animals[idx].homeInfo;
...
void mNpcW_SetGoalBlock(NpcWalkInfo* info) {
    ...
    switch (info->type) {
        ...
        case NPCW_GOAL_MY_HOME:
            ...
            info->goalX = common_data.save.flash.animals[idx].homeInfo.blockX;
            info->goalZ = common_data.save.flash.animals[idx].homeInfo.blockZ;
            break;
...
void mNpcW_InitNpcWalk(NpcWalking* walk) {
    ...
    animals = common_data.save.flash.animals;
    walk->infoMax = NPCW_GET_WALK_NUM(common_data.save.flash.nowNpcMax);

    for (i = 0; i < walk->infoMax; i++) {
        idx = mNpcW_DecideNpc(animals, walk->idxUse);

        mNpcW_SetNpcWalkInfo(info, &common_data.save.flash.animals[idx], idx);
        walk->idxUse |= (1 << idx);
        info++;
    }
...
s32 mNpcW_GetWalkInfoStatusGoalAnimalIdx(s32* status, s32* goal, s32 idx) {
    ...
    *goal = NPCW_GOAL_NUM;
    ...
    if (idx >= 0 && idx < ANIMAL_NUM_MAX) {
        NpcWalking* walk = &common_data.npcWalk;
        s32 info_idx = mNpcW_GetNpcWalkInfoIdx(walk->info, NPCW_MAX, &common_data.save.flash.animals[idx].id);
