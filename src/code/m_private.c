...
s32 mPr_GetPlayerName(char* buf, s32 playerNumber) {
    ...
    if (mLd_PlayerManKindCheckNo(playerNumber) == FALSE) {
        if (mPr_NullCheckPersonalID(&common_data.save.flash.saveFilePrivateInfo[playerNumber].playerId) == FALSE) {
            mPr_CopyPlayerName(buf, common_data.save.flash.saveFilePrivateInfo[playerNumber].playerId.playerName);
            res = TRUE;
        }
...
void mPr_InitPrivateInfo(PrivateInfo* priv) {
    ...
    priv->playerId.landId = common_data.save.flash.landInfo.id;
    mLd_CopyLandName(priv->playerId.landName, common_data.save.flash.landInfo.name);

    pid = RANDOM_F(253);
    ogPriv = common_data.save.flash.saveFilePrivateInfo;
...
s32 mPr_LoadPak_and_SetPrivateInfo2(u8 player, PakInfo* pak) {
    ...
    if (save == TRUE) {
        if (player < 4) {
            priv = &common_data.save.flash.saveFilePrivateInfo[player];
...
s32 mPr_SendMotherMailPost(PersonalID_c* pid, s32 playerNumber, u16 present, s32 stationery, s32 mailNumber) {
    ...
    mail = &l_mpr_mail;
    home = &common_data.save.flash.homes[mHS_get_arrange_idx(playerNumber)];
...
void mPr_SendMailFromMother() {
    ...
    if (mLd_PlayerManKindCheckNo(playerNumber) == FALSE && priv != NULL &&
        ...
        mailDate = &common_data.save.flash.motherMailInfo[playerNumber].date;
        ...
        if (mailDate->year != mTM_rtcTime_ymd_clear_code.year && mailDate->month != mTM_rtcTime_ymd_clear_code.month &&
            ...
            if (mailDate->year != rtcTime->year || mailDate->month != rtcTime->month || mailDate->day != rtcTime->day) {
                mPr_SendMotherMail(&common_data.save.flash.motherMailInfo[playerNumber], rtcTime);
            }
        } else {
...
s32 mPr_CheckFishCompleteTalk(u8 playerNumber) {
    ...
    if (playerNumber < PLAYER_NUM &&
        mPr_GetCompleteTalk(common_data.save.flash.saveFilePrivateInfo[playerNumber].completeFishInsectFlags, 0) ==
            TRUE) {
        res = TRUE;
    }
...
s32 mPr_CheckInsectCompleteTalk(u8 playerNumber) {
    ...
    if (playerNumber < PLAYER_NUM &&
        mPr_GetCompleteTalk(common_data.save.flash.saveFilePrivateInfo[playerNumber].completeFishInsectFlags, 1) ==
            TRUE) {
        res = TRUE;
    }
...
void mPr_SetMapThisLand(mPr_map_info_c* mapInfo) {
    if (mapInfo != NULL) {
        mLd_CopyLandName(mapInfo->landName, common_data.save.flash.landInfo.name);
        mapInfo->landId = common_data.save.flash.landInfo.id;
    }
}
...
s32 mPr_GetThisLandMapIdx(mPr_map_info_c* map_info, s32 max) {
    return mPr_GetLandMapIdx(map_info, max, &common_data.save.flash.landInfo);
}
...
void mPr_PrintMapInfo_debug(gfxprint* gfxprint) {
    ...
    if (priv != NULL) {
        ...
        gfxprint_printf(gfxprint, "%04x ", common_data.save.flash.landInfo.id);
