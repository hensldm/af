...
void mSN_ClearSnowmanData(u16* name, s32 snowmanIndex) {
    bzero(&common_data.save.flash.snowmanData[snowmanIndex], sizeof(SnowmanData));
    *name = 0;
}
...
s32 mSN_MeltSnowman(u16* name, s32 daysElapsed) {
    ...
    if ((*name >= SNOWMAN0) && (*name <= SNOWMAN8)) {
        ...
        if (daysElapsed < 0) {
            daysElapsed = 1;
            common_data.save.flash.snowmanYear = 0;
            common_data.save.flash.snowmanMonth = 0;
            common_data.save.flash.snowmanDay = 0;
            common_data.save.flash.snowmanHour = 0;
        }
        ...
        if (!mSN_check_life(name, daysElapsed)) {
            ...
            SnowmanData* snowmanData = &common_data.save.flash.snowmanData[snowmanIndex];
...
s32 mSN_get_free_space(void) {
    SnowmanData* snowmanData = common_data.save.flash.snowmanData;
    s32 snowmanIndex;
...
void mSN_regist_snowman_society(SnowmanInfo* snowmanInfo) {
    ...
    if (snowmanIndex != -1) {
        mem_copy((u8*)&common_data.save.flash.snowmanData[snowmanIndex], (u8*)snowmanInfo, sizeof(SnowmanData));
...
void mSN_snowman_init(void) {
    bzero(common_data.save.flash.snowmanData, sizeof(SnowmanData[SNOWMAN_SAVE_COUNT]));
    mSN_decide_msg();
}
