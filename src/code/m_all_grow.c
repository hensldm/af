...
void mAGrw_SetDebugData() {
    Foreground* fgItems = common_data.save.flash.foreground[0];
    u16* deposit = common_data.save.flash.deposit[0];
    s32 blockZ;
...
s32 mAGrw_CheckKabuPeddler() {
    ...
    lbRTC_time_c* renewTime = &common_data.save.flash.allGrowRenewTime;
...
void mAGrw_OrderSetHaniwa() {
    common_data.save.flash.haniwaScheduled = TRUE;
}
...
void mAGrw_XmasTreeField(s32 type) {
    ...
    Foreground* block = common_data.save.flash.foreground[0];
    s32 i;
...
void mAGrw_SetXmasTree() {
    if (common_data.save.flash.sceneNo == SCENE_FG) {
...
void mAGrw_RenewalFgItem(lbRTC_time_c* time) {
    ...
    if (common_data.save.flash.sceneNo == SCENE_FG) {
        s32 schedHaniwa = common_data.save.flash.haniwaScheduled;
        ...
        if (D_80100C5C_jp != NULL) {
            ...
            common_data.save.flash.haniwaScheduled = schedHaniwa;
...
void mAGrw_ChangeTree2FruitTree() {
    s32 fruit = common_data.save.flash.fruit - 0x2800;
    u16 fruitTree;
    s32 blockZ;

    fruitTree = l_tree_max_table[fruit];
    for (blockZ = 0; blockZ < FG_BLOCK_Z_NUM; blockZ++) {
        mAGrw_ChangeTree2FruitTreeLine(common_data.save.flash.foreground[blockZ], fruitTree);
    }
}
