...
s32 mCkRh_InitGokiSaveData_1Room(s32 player) {
    common_data.save.flash.homes[player & 3].goki.num = 0;
    common_data.save.flash.homes[player & 3].goki.pad = 0;
    common_data.save.flash.homes[player & 3].goki.time.year = common_data.time.rtcTime.year;
    common_data.save.flash.homes[player & 3].goki.time.month = common_data.time.rtcTime.month;
    common_data.save.flash.homes[player & 3].goki.time.day = common_data.time.rtcTime.day;
}
...
void mCkRh_SavePlayTime(s32 player) {
    ...
    if (player < PLAYER_NUM) {
        ...
        common_data.save.flash.homes[home].goki.time.year = common_data.time.rtcTime.year;
        common_data.save.flash.homes[home].goki.time.month = common_data.time.rtcTime.month;
        common_data.save.flash.homes[home].goki.time.day = common_data.time.rtcTime.day;
    }
}
...
s32 mCkRh_DaysGapCompareWithSaveTime(s32 player) {
    ...
    if (player < PLAYER_NUM) {
        ...
        home = mHS_get_arrange_idx(player) & 3;
        gokiTime.year = common_data.save.flash.homes[home].goki.time.year;
        gokiTime.month = common_data.save.flash.homes[home].goki.time.month;
        gokiTime.day = common_data.save.flash.homes[home].goki.time.day;
...
void mCkRh_DecideNowGokiFamilyCount(s32 player) {
    ...
    if (player < PLAYER_NUM) {
        ...
        if (dayGap > 6) {
            s32 gokiNum;
            s32 count;
            gokiNum = common_data.save.flash.homes[home].goki.num;
            count = gokiNum > 0 ? dayGap : dayGap - 6;
            common_data.save.flash.homes[home].goki.num = mCkRh_GokiFamilyCount2Good(count + gokiNum);
        }
    }
}
...
s32 mCkRh_PlussGokiN_NowRoom(s32 count) {
    ...
    if (FI_IS_PLAYER_ROOM(fieldId)) {
        ...
        if ((player < PLAYER_NUM) && (housefieldId == homeId)) {
            num = common_data.save.flash.homes[homeId].goki.num;

            common_data.save.flash.homes[homeId].goki.num = mCkRh_GokiFamilyCount2Good(count + num);
            return TRUE;
        }
    }
    return FALSE;
...
s32 mCkRh_MinusGokiN_NowRoom(s32 count) {
    ...
    if (FI_IS_PLAYER_ROOM(fieldId)) {
        ...
        if ((player < PLAYER_NUM) && (housefieldId == homeId)) {
            num = common_data.save.flash.homes[homeId].goki.num;
            common_data.save.flash.homes[homeId].goki.num = mCkRh_GokiFamilyCount2Good(num - count);
            return TRUE;
        }
    }
    return FALSE;
...
s32 mCkRh_NowSceneGokiFamilyCount() {
    ...
    if (FI_IS_PLAYER_ROOM(fieldid)) {
        return common_data.save.flash.homes[FI_GET_PLAYER_ROOM_NO(fieldid)].goki.num;
    } else {
        return 0;
    }
}
