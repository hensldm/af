...
Actor* aSTR_setupActor_proc(Game_Play* game_play, u16 structureName, f32 posX, f32 posZ, s16 params) {
    ...
    if (structureName < 0x50DA) {
        s32 index = mNpc_SearchAnimalinfo(common_data.save.flash.animals, structureName + 0x9000, 0xF);
        ...
        structureType = common_data.save.flash.homes[index].unk_022.unk_22_0 + STRUCTURE_TYPE_MY_HOUSE_1;
        paletteType = common_data.save.flash.homes[index].unk_024 + STRUCTURE_PALETTE_MY_HOUSE_1;
    } else if ((structureName >= 0x5809) && (structureName < 0x580A)) {
        s32 stationType = common_data.save.flash.stationType;
