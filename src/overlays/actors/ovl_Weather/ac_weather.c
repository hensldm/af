...
s32 aWeather_StartSysLevSE() {
    ...
    if (this->soundFlag == 1) {
        if (ENV_SAVE_GET_WEATHER_TYPE(common_data.save.flash.saveWeather) == 1) {
            switch (ENV_SAVE_GET_WEATHER_INTENSITY(common_data.save.flash.saveWeather)) {
                case 1:
...
void aWeather_ChangeEnvSE(Weather* this, Game_Play* play, s16 status, s16 level) {
    ...
    if (mEv_CheckTitleDemo() <= 0) {
        if (common_data.save.flash.sceneNo == SCENE_START_DEMO || common_data.save.flash.sceneNo == SCENE_START_DEMO2) {
            return;
        }
...
void aWeather_ChangeWeatherTime0(Weather* this) {
    ...
    if ((mEv_CheckTitleDemo() <= 0)) {
        if ((common_data.save.flash.sceneNo == 15) || (common_data.save.flash.sceneNo == 16)) {
            return;
        }
        if ((mTM_check_renew_time(0) != 0)) {
            ...
            saveWeather = ENV_SAVE_GET_WEATHER_TYPE(common_data.save.flash.saveWeather);
            ...
            common_data.save.flash.saveWeather = saveRnd;
